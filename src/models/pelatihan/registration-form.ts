...
import { noWaCheck } from "@/server/services/pelatihan/no-wa-available-check";
import { emailCheck } from "@/server/services/pelatihan/email-check";
import { npmCheck } from "@/server/services/pelatihan/npm-check";
import { kelasCheck } from "@/server/services/pelatihan/kelas-check";
...
export const participantAsMahasiswa = z
    .object({
        ...
        npm: z
            ...
            .refine(
                (npm) => {
                    console.log(isNumeric(npm));
                    return isNumeric(npm);
                },
                {
                    error: `NIM/NPM tidak validsss!.`,
                }
            )
            .refine(
                async (noWa) => {
                    const res = await npmCheck(noWa);
                    return res.success;
                },
                {
...
export const classRegstrationSchema = z
    .object({
        kelas: z.string("Kelas harus diisi!.")
            .refine(
                async (kelas) => {
                    const res = await kelasCheck(kelas);
                    return res.success;
                },
                {
                    error: "Kelas telah melakukan pendaftaran.",
                    path: ["kelas"],
                }
            ),
        nominal: z
        ...
        participants: z.array(
            z
                ...
                .refine(
                    async (data) => {
                        const res = await npmCheck(data.npm);
                        return res.success;
                    },
                    {
                        error: "NIM/NPM telah terdaftar!.",
