...
export async function exportSemuaDataPesertaSeminar(): Promise<ServerResponseType<{data: string, filename: string }>> {
  try {
    ...
    if (!allParticipants || allParticipants.length === 0) {
      return { success: false, message: 'Tidak ada data peserta untuk diekspor.' };
    }
    ...
    return { success: true,data: { data: base64String, filename: "Data_Peserta_Seminar.xlsx"} };
