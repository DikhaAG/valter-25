...
import { FormPendaftaranPesertaSchemaType } from "@/zod/home/video-campaign/form-pendaftaran-tim-schema";
...
export async function cekKetersediaanPemain(
   ...
   for (const p of peserta) {
      try {
         ...
         const cekNpmRes = await db.query.pesertaVideoCampaignTable.findFirst({
            where: eq(pesertaVideoCampaignTable.npm, p.npm!),
         });
         if (cekNpmRes) {
            return {
               success: false,
