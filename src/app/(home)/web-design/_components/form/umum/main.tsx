...
import { webDesignUmumSubmitFormAction } from "@/server/home/web-design/insert/umum-submit-form-action";

export function WebDesignUmumForm() {
   const [termsChecked, setTermsChecked] = useState<boolean>(false);
   const [loading, setLoading] = useState(false);
   const router = useRouter();
   const form = useForm<WebDesignTimUmumFormSchemaType>({
      resolver: zodResolver(webDesignTimUmumFormSchema),
      defaultValues: {
         namaTim: "",
         noWa: "",
   ...
   async function onSubmit(data: WebDesignTimUmumFormSchemaType) {
      setLoading(true);

      const res = await webDesignUmumSubmitFormAction(data);
      if (!res.success) {
         CustomToast({
            variant: "warning",
            message: `${res.message} 😂💀`,
