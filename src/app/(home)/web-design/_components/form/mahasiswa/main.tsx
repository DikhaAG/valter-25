...
import { webDesignPesertaNpmAvailableCheck } from "@/server/home/web-design/validate/npm-available-check";
import { webDesignMahasiswaSubmitFormAction } from "@/server/home/web-design/insert/mahasiswa-submit-form-action";

export function WebDesignMahasiswaForm() {
   const [termsChecked, setTermsChecked] = useState<boolean>(false);
   const [loading, setLoading] = useState(false);
   const router = useRouter();
   const form = useForm<WebDesignTimMahasiswaFormSchemaType>({
      resolver: zodResolver(webDesignTimMahasiswaFormSchema),
      defaultValues: {
         namaTim: "",
         instansi: "",
         noWa: "",
   ...
   async function onSubmit(data: WebDesignTimMahasiswaFormSchemaType) {
      setLoading(true);

      let hasError = false;
      for (const [i, p] of data.peserta.entries()) {
         const cekNpm = await webDesignPesertaNpmAvailableCheck(p);
         if (!cekNpm.success) {
      ...
      const res = await webDesignMahasiswaSubmitFormAction(data);
      if (!res.success) {
         CustomToast({
            variant: "warning",
            message: `${res.message} 😂💀`,
         ...
         >
            ...
            <div className="flex justify-end">
               <Button
                  ...
                  disabled={form.getValues("peserta").length === 2}
                  variant="gosong"
