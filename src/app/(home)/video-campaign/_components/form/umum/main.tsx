...
import { videoCampaignUmumSubmitFormAction } from "@/server/home/video-campaign/insert/umum-submit-form-action";

export function VideoCampaignUmumForm() {
   const [termsChecked, setTermsChecked] = useState<boolean>(false);
   const [loading, setLoading] = useState(false);
   const router = useRouter();
   const form = useForm<VideoCampaignTimUmumFormSchemaType>({
      resolver: zodResolver(videoCampaignTimUmumFormSchema),
      defaultValues: {
         namaTim: "",
         noWa: "",
   ...
   async function onSubmit(data: VideoCampaignTimUmumFormSchemaType) {
      setLoading(true);

      const res = await videoCampaignUmumSubmitFormAction(data);
      if (!res.success) {
         CustomToast({
            variant: "warning",
            message: `${res.message} 😂💀`,
