...
import { videoCampaignPesertaNpmAvailableCheck } from "@/server/home/video-campaign/validate/npm-available-check";
import { videoCampaignMahasiswaSubmitFormAction } from "@/server/home/video-campaign/insert/mahasiswa-submit-form-action";

export function VideoCampaignMahasiswaForm() {
   const [termsChecked, setTermsChecked] = useState<boolean>(false);
   const [loading, setLoading] = useState(false);
   const router = useRouter();
   const form = useForm<VideoCampaignTimMahasiswaFormSchemaType>({
      resolver: zodResolver(videoCampaignTimMahasiswaFormSchema),
      defaultValues: {
         namaTim: "",
         instansi: "",
         noWa: "",
   ...
   async function onSubmit(data: VideoCampaignTimMahasiswaFormSchemaType) {
      setLoading(true);

      let hasError = false;
      for (const [i, p] of data.peserta.entries()) {
         const cekNpm = await videoCampaignPesertaNpmAvailableCheck(p);
         if (!cekNpm.success) {
      ...
      const res = await videoCampaignMahasiswaSubmitFormAction(data);
      if (!res.success) {
         CustomToast({
            variant: "warning",
            message: `${res.message} 😂💀`,
