...
import { getWebDesignIncome } from "@/server/services/admin/getIncome";
import {
   getTotalTimWebDesignBelumTerkonfirmasi,
   getTotalTimWebDesignTerkonfirmasi,
} from "@/server/services/admin/getTotalPeserta";
import { formatIDRCurrency } from "@/utils/formatIDRCurrency";
import { useEffect, useState } from "react";

export function WebDesignSectionCards() {
   ...
   useEffect(() => {
      getWebDesignIncome().then((res) => {
         if (res.success) {
            setIncome(res.data);
         }
      });

      getTotalTimWebDesignTerkonfirmasi().then((res) => {
         if (res.success) {
            setTotalPendaftarTerkonfirmasi(res.data);
         }
      });

      getTotalTimWebDesignBelumTerkonfirmasi().then((res) => {
         if (res.success) {
