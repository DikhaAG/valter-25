...
import { getEsportIncome } from "@/server/services/admin/getIncome";
import {
   getTotalTimEsportBelumTerkonfirmasi,
   getTotalTimEsportTerkonfirmasi,
} from "@/server/services/admin/getTotalPeserta";
import { formatIDRCurrency } from "@/utils/formatIDRCurrency";
import { useEffect, useState } from "react";

export function EsportSectionCards() {
   ...
   useEffect(() => {
      getEsportIncome().then((res) => {
         if (res.success) {
            setIncome(res.data);
         }
      });

      getTotalTimEsportTerkonfirmasi().then((res) => {
         if (res.success) {
            setTotalPendaftarTerkonfirmasi(res.data);
         }
      });

      getTotalTimEsportBelumTerkonfirmasi().then((res) => {
         if (res.success) {
