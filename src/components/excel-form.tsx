...
const formSchema = z.object({
   ...
   nominalBayar: z
      .number({ error: (error) => `eror ${typeof error.input}` })
      .min(60000),
   ...
   rows: z.array(
      z.object({
         email: z
            .email({ error: (e) => `${typeof e.input}` })
            .min(1, "email tidak boleh kosong")
            .transform((val) => String(val)),
         nama: z.string().min(1, "Nama tidak boleh kosong"),
         noWa: z
            .string({ error: (e) => `${typeof e.input}` })
            .min(1, "no wa tidak boleh kosong")
            .transform((val) => String(val)).refine((noWa) => {
              return isNumeric(noWa)
            }, {error: "nomor harus nomor!", path: ["rows"]}),
      })
   ),
});
   ...
   const handleProcessFile = async () => {
      ...
      try {
         ...
         if (json.length > 0) {
            ...
            const requiredColumns = ["email", "noWa", "nama"];
   ...
   const onSubmit = async (data: FormValues) => {
      ...
      if (nominalPerPeserta === biayaPendaftaran) {
         ...
         toast.error("Pembayaran Gagal", {
            description:
               "Jumlah peserta yang daftar tidak sesuai dengan total nominal yang diberikan.",
         });
      } else if (nominalPerPeserta > biayaPendaftaran) {
         toast.error("Pembayaran Gagal", {
         ...
         toast.error("Pembayaran Gagal", {
            description:
               "Terjadi kesalahan dalam perhitungan. Periksa kembali nominal dan data excel.",
         });
      }
   };
