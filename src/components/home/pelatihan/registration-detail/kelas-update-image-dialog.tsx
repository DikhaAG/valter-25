...
import { updateBuktiPembayaranKelas } from "@/server/services/pelatihan/update-bukti-pembayaran-kelas";

// --- KOMPONEN DIALOG UPDATE GAMBAR ---
interface Props {
   data: ClassRegistrationTable;
}
export const KelasUpdateImageDialog = ({ data }: Props) => {
   /**
   ...
   const handleUpdate = async () => {
      ...
      if (!isValidImageType(file) || !isFileSizeValid(file)) {
         toast.error("File tidak valid. Pastikan JPG/JPEG/PNG dan maks 500KB.");
         return;
      }

      startTransition(async () => {
         // Memanggil Server Action untuk mengunggah dan memperbarui data
         const res = await updateBuktiPembayaranKelas({
            file: file,
            kelas: data.kelas,
         });
      ...
      <Dialog open={open} onOpenChange={setOpen}>
         ...
         <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
               ...
               <DialogDescription>
                  Pilih gambar baru untuk bukti pembayaran {data.kelas}.
               </DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
               <ImageUploadField onFileChange={setFile} />
               {file && (
            ...
            <div className="flex justify-end">
               <Button onClick={handleUpdate} disabled={isPending || !file}>
                  {isPending && (
