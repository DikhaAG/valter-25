...
import ProdiSelector from "@/components/home/pelatihan/form-registration/prodi-selector";
...
export function KelasForm() {
  ...
  const form = useForm<ClassRegstrationSchema>({
    ...
    defaultValues: { kelas: undefined, nominal: 0, excelFile: undefined, participants: [] },
    mode: "all",
  });
                ...
                <FormControl className="">
                  <ProdiSelector
                    ...
                    onKelasChange={async (kelas) => {
                      if (kelas) {
                        const isKelasAvalable = await kelasCheck(kelas.nama)
                        if (!isKelasAvalable.success) {
                          form.setError("kelas", { message: isKelasAvalable.message })
                        }
                      }
          ...
          <div className="max-h-screen">
            ...
            <div className="space-y-4">
              {watchedRows.map((row, index) => (
                <div
                  key={index}
                  className="border-4 border-dashed p-4 rounded-md"
                >
                    ...
                    {Object.keys(row as MahasiswaExcelRow).map(
                      (key) => (
                        <FormField
                          key={key}
                          control={form.control}
                          name={`participants.${index}.${key as keyof MahasiswaExcelRow
                            }`}
                          render={({ field }) => (
                            <FormItem>
