...
import { pesertaVideoCampaignTableSchema } from "@/zod/tables/video-campaign/peserta";
import { z } from "zod";

/**
 * Skema validasi untuk data tampilan (display) tim lomba web design.
 * Tipe ini digunakan saat mengambil dan menampilkan data tim dari database,
 * bukan untuk validasi formulir pendaftaran.
 */
export const videoCampaignRegistrationDisplaySchema = z.object({
   as: asEnumSchema,
   /**
    * ID unik tim.
    ...
    * Tipe datanya divalidasi oleh `pesertaVideoCampaignTableSchema`.
    */
   peserta: z.array(pesertaVideoCampaignTableSchema),
});
 ...
 */
export type VideoCampaignRegistrationDisplaySchemaType = z.infer<
   typeof videoCampaignRegistrationDisplaySchema
>;
